...
class Albaheth(qt.QWidget):
    def __init__(self):
        ...
        self.setStyleSheet("""            /* Push Button Styles */
            QPushButton {
                background-color: #007bff; /* A vibrant blue */
                color: white;
                border: none; /* No border for a cleaner look */
                border-radius: 6px;
                padding: 10px 15px; /* More padding for bigger buttons */
                min-height: 40px;
                font-weight: bold;
            ...
            QPushButton:pressed {
                background-color: #003d80; /* Even darker blue on pressed */
            }
            /* Specific button styles (using objectName) */
            QPushButton#searchModeButton {
                background-color: #0056b3; /* Darker blue, as requested */
            }
            QPushButton#searchModeButton:hover {
                background-color: #003d80;
            }
            QPushButton#searchModeButton:pressed {
                background-color: #003d80;
            }
            ...
            QPushButton#applySearchModeChangesButton {
                background-color: #28a745; /* Green for apply - same as start button */
            }
            QPushButton#applySearchModeChangesButton:hover {
                background-color: #218838;
            }
            QPushButton#applySearchModeChangesButton:pressed {
                background-color: #218838;
            }
            ...
            /* Style for the delete results button */
            QPushButton#clearResultsButton {
                background-color: #dc3545; /* Dark red - same as cancel button */
                color: white;
                border: none;
                border-radius: 6px;
                padding: 10px 15px;
            ...
            QPushButton#clearResultsButton:pressed {
                background-color: #bd2130; /* Even darker red on pressed */
            }
        """)        
        self.init_ui()
    ...
    def onSearchClicked(self):
        ...
        if index == 0:
            ...
            if result:
                ...
                guiTools.qMessageBox.MessageBox.view(self,"تنبيه","لم يتم العثور على نتائج")
                self.clear_results_button.setDisabled(True)
        else:
            try:
                ...
                if isinstance(ahadeeth_data, list):
                    ...
                    guiTools.qMessageBox.MessageBox.error(self, "خطأ في البيانات", "تنسيق ملف الأحاديث غير صحيح.")
            except Exception as e:
                guiTools.qMessageBox.MessageBox.error(self, "خطأ غير متوقع", f"حدث خطأ أثناء تحميل الأحاديث: {e}")        
    ...
    def go_to_surah(self, metadata):        
        ...
        if surah_name_key in self.surahsList:
            ...
            guiTools.qMessageBox.MessageBox.error(self, "خطأ", f"لم يتم العثور على بيانات السورة: {surah_name_key}")                
        self.resume_after_action()    
    def show_ayah_info(self, metadata):        
        ...
        guiTools.qMessageBox.MessageBox.view(self, "معلومات الآية", info_text)                
        self.resume_after_action()    
    ...
    def copy_line(self):
        try:
            ...
            guiTools.qMessageBox.MessageBox.error(self, "تنبيه حدث خطأ", str(e))
