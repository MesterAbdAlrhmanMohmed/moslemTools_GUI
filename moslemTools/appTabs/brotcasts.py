...
class SchedulingDialog(qt.QDialog):
    def __init__(self, parent):
        ...
        layout.addWidget(self.start_h_label)
        ...
        layout.addWidget(self.start_s_spin)
        line = qt.QFrame()
        line.setFrameShape(qt.QFrame.Shape.HLine)
        ...
        self.dur_label.setStyleSheet("font-weight: bold;")
        ...
        layout.addWidget(self.dur_h_label)
        ...
        layout.addWidget(self.dur_s_spin)
...
class protcasts(qt.QWidget):
    ...
    def recordingError(self, error_msg):
        ...
        if not self.startBtn.isEnabled() or self.countdown_timer.isActive():
            guiTools.qMessageBox.MessageBox.error(self, "خطأ في التسجيل", f"حدث خطأ: {error_msg}")
        self.resetRecorderState()
