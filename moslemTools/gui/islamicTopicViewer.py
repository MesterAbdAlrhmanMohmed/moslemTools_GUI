...
class IslamicTopicViewer(qt.QDialog):
    ...
    def OnContextMenu(self):
        ...
        if note_exists:
            ...
            view_note_action.triggered.connect(lambda: self.onNoteAction(position_data))            
            delete_note_widget_action = qt.QWidgetAction(self)
            delete_note_button = guiTools.QPushButton("حذف ملاحظة السطر الحالي")
            ...
            delete_note_button.clicked.connect(lambda: self.onDeleteNote(position_data))
        ...
        if state:            
            ...
            remove_bookmark_button = guiTools.QPushButton("حذف العلامة المرجعية الحالية")
