...
class TafaseerViewer(qt.QDialog):
    def __init__(self, p, From, to):
        ...
        self.current_tafaseer_label = qt.QLineEdit(f"التفسير المحدد هو: {functions.tafseer.getTafaseerByIndex(self.index)}")
        self.current_tafaseer_label.setReadOnly(True)
        ...
        bottomLayout.addStretch(1)
        self.warning_label = qt.QLineEdit("تنبيه: إذا غيرت التفسير ولم يظهر النص، اختر نفس التفسير مرة أخرى.")
