...
class translationViewer(qt.QDialog):
    def __init__(self, p, From, to):
        ...
        self.current_translation_label = qt.QLineEdit(f"الترجمة المحددة هي: {functions.translater.gettranslationByIndex(self.index)}")
        self.current_translation_label.setReadOnly(True)
        ...
        fontLayout.addWidget(self.more_options_label)
        ...
        bottomLayout.addStretch(1)
        self.warning_label = qt.QLineEdit("تنبيه: إذا غيرت الترجمة ولم يظهر النص، اختر نفس الترجمة مرة أخرى.")
        ...
        bottomLayout.addWidget(self.warning_label, 1)
        bottomLayout.addSpacing(15)
