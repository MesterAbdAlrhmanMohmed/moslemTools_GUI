...
class QuranViewer(ContextMenuMixin, NavigationDisplayMixin, AudioPlayerMixin, SearchHandlerMixin, MergerSaverMixin, NotesBookmarksMixin, TafseerAndInfoMixin, qt.QDialog):
    def __init__(self,p,text:str,type:int,category,index=0,enableNextPreviouseButtons=False,typeResult=[],CurrentIndex=0,enableBookmarks=True):
        ...
        self.audioOutput.setDevice(audio_manager.get_audio_device("quran_viewer"))
        self.audioOutput.setVolume(self.load_volume())
        ...
        self.toggle_search_button = guiTools.QPushButton(self.get_search_button_text())
        ...
        qt1.QShortcut("ctrl+shift+m", self).activated.connect(self.set_volume_dialog)
        ...
        qt1.QShortcut("ctrl+shift+e", self).activated.connect(self.getQiraatForSurah)
        qt1.QShortcut("shift+alt+e", self).activated.connect(self.getQiraatFromAyahToEnd)
        qt1.QShortcut("ctrl+alt+e", self).activated.connect(self.QiraatFromVersToVers)
