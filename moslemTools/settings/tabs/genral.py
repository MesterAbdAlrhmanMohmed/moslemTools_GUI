...
class Genral(qt.QWidget):
    def __init__(self, p):
        ...
        self.exit_note = qt.QLineEdit("في حالة إلغاء تحديد المربع أعلاه، سيقوم البرنامج بالإغلاق مباشرة دون عرض نافذة خيارات الخروج")
        self.exit_note.setReadOnly(True)
        ...
        delete_laybol = qt.QLineEdit("لحذف القارئ المحدد قم بالضغط على زر الحذف أو زر التطبيقات")
        delete_laybol.setReadOnly(True)
        ...
        self.tray_note = qt.QLineEdit("تنبيه هام، يمكنكم إظهار أو إخفاء البرنامج عبر استخدام الاختصار windows+alt+h أو من قائمة علبة النظام system tray")
        self.tray_note.setReadOnly(True)
        ...
        self.run_note = qt.QLineEdit("تنبيه هام، يمكنكم تشغيل البرنامج من قائمة run بكتابة الأمر (mt) أو الاختصار (ctrl+alt+m)")
        self.run_note.setReadOnly(True)
