...
class MotonPlayerSettings(qt.QWidget):
    def __init__(self, p):
        super().__init__()
        self.setStyleSheet("""
        ...
        self.times_laybol = qt.QLabel("عدد مرات تكرار الأبيات")
        self.times = qt.QSpinBox()
        self.times.setRange(1, 10)
        self.times.setValue(int(settings_handler.get("motonPlayer", "times") or 1))
        self.times.setAccessibleName("عدد مرات تكرار الأبيات")
        self.times.setMinimumWidth(100)
        ...
        self.replay = qt.QCheckBox("إعادة التشغيل بعد الانتهاء: في حالة اختيار التشغيل إلى النهاية، أو من بيت إلى بيت")
        self.replay.setChecked(p.cbts(settings_handler.get("motonPlayer", "replay") or "False"))
